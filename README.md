...
Bốn nhóm tấn công được nghiên cứu:

| Hình thức tấn công | Đặc điểm nhận diện | Công cụ tiêu biểu (PAI) |
|---|---|---|
| Print Attack | Ảnh tĩnh 2D, thiếu thông tin khối | Ảnh màu, giấy A4, Poster |
| Replay Attack | Video qua màn hình, nhiễu tần số | Smartphone, Tablet, Laptop |
| Mask Attack | Vật thể 3D giả lập da và khối mặt | Mặt nạ Silicon, Latex, nhựa 3D |
| Digital Attack | Thao túng dữ liệu bằng AI | Deepfake, FaceSwap, GANs |

!![Uploading image.png…]()


Điểm khác biệt chính của nghiên cứu:
...
![Hình 4.1 — So sánh các chỉ số phân loại nhị phân của ba backbone](images/hinh_4_1_so_sanh_chi_so.png)
...
![Hình 4.6 — Ma trận nhầm lẫn chuẩn hóa của ba backbone trên tập test](images/hinh_4_6_confusion_matrix.png)
...
![Hình 4.7 — Recall theo từng loại tấn công của ba backbone](images/hinh_4_7_recall_loai_tan_cong.png)
...
![Hình 2.1 — Thống kê số lượng và tỉ lệ Live/Spoof trong tập dữ liệu tổng hợp](images/hinh_2_1_thong_ke_live_spoof_bar.png)
...
![Hình 2.2 — Pipeline tiền xử lý dữ liệu](images/hinh_2_2_pipeline_tien_xu_ly.png)
...
![Hình 2.3 — Ví dụ các mẫu Spoof (trái) và Live đã cân bằng chất lượng (phải)](images/hinh_2_3_vi_du_spoof_live.png)
...
![Hình 2.4 — Dữ liệu sau khi chia train/test](images/hinh_2_4_chia_train_test.png)
...
Kích thước 512 được chọn sau thử nghiệm 256/512/1024: 512 cho khoảng cách train-val loss nhỏ nhất (~0.08–0.10), hội tụ mượt nhất. 256 gây suy giảm nhẹ (val loss cao hơn ~0.05); 1024 gây overfitting sớm (khoảng cách train-val loss ~0.15).
...
![Hình 3.1 — Kiến trúc mô hình ConvNeXt](images/hinh_3_1_kien_truc_convnext.png)
...
Regularization đặc thù: Dropout 0.6 + Mixup augmentation + Label Smoothing 0.2.

![Hình 3.2 — Kiến trúc mô hình EfficientNet-V2-B0](images/hinh_3_2_kien_truc_efficientnet.png)

### ViT-Base + LoRA

Vision Transformer với Parameter-Efficient Fine-Tuning. Ảnh 224x224 chia patch 16x16, chuỗi **197 token** (196 patch + 1 class) qua 12 lớp Transformer Encoder. **86M tham số gốc**, chỉ **0.6M trainable** (LoRA rank=16, alpha=32 trên Query và Value).

Head: `Linear(768, 512) -> LayerNorm -> GELU -> Dropout -> Linear(512, 1)`

![Hình 3.3 — Kiến trúc mô hình ViT-Base](images/hinh_3_3_kien_truc_vit.png)
...
- **WeightedRandomSampler**: trọng số Live=3.0, Spoof=1.0 để duy trì ~50/50 Live/Spoof trong mỗi batch.
...
| Backbone | Tốc độ hội tụ | Đặc điểm Loss | Plateau Accuracy | Epoch dừng |
|---|---|---|---|---|
| ConvNeXt-Tiny | Nhanh nhất (17 epoch) | Val < Train (~0.22 vs ~0.25) | ~94.85% (epoch 12) | 17 |
| EfficientNet-V2-B0 | Trung bình (16–17 epoch) | Song hành, khoảng cách ~0.10 | **99.07%** (epoch 16) | 28 |
| ViT-Base-LoRA | Chậm nhất (22+ epoch) | Dao động ±0.03–0.05, Val ~0.27 | ~87% (chưa rõ plateau) | 22 |

![Hình 4.2 — Diễn biến quá trình huấn luyện của ConvNeXt-Tiny](images/hinh_4_2_learning_curve_convnext.png)

![Hình 4.3 — Diễn biến quá trình huấn luyện của EfficientNet-V2-B0](images/hinh_4_3_learning_curve_efficientnet.png)

![Hình 4.4 — Diễn biến quá trình huấn luyện của ViT-Base](images/hinh_4_4_learning_curve_vit.png)

---

## Phân Bố Probability Score

![Hình 4.5 — Phân bố điểm xác suất dự đoán thật/giả của ba backbone trên tập kiểm định](images/hinh_4_5_phan_bo_xac_suat.png)

- **EfficientNet-V2-B0**: hai đỉnh tách biệt hoàn toàn (Live tập trung 0.0–0.2, Spoof tập trung 0.9–1.0), vùng giữa 0.3–0.8 gần như trống — phân bố lý tưởng nhất.
- **ConvNeXt-Tiny**: hai đỉnh rõ nhưng vẫn có một số mẫu ở vùng biên, giải thích accuracy 94.85%.
- **ViT-Base-LoRA**: vùng giữa 0.2–0.8 có nhiều mẫu chồng lấn, mô hình còn "do dự" nhiều.

---

## Các Trường Hợp Thất Bại Điển Hình

![Hình 4.8 — Các trường hợp thất bại điển hình (phần 1)](images/hinh_4_8_that_bai_dien_hinh_1.png)

![Hình 4.8 — Các trường hợp thất bại điển hình (phần 2)](images/hinh_4_8_that_bai_dien_hinh_2.png)

Các failure case đặc trưng: print/replay chất lượng cao với ánh sáng phẳng, độ tương phản thấp hoặc nền phức tạp. Mẫu dự đoán đúng với độ tự tin cao thường là live trong môi trường ánh sáng tự nhiên và spoof có mép giấy/mặt nạ hoặc moiré màn hình rõ rệt.
