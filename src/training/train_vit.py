...
def main():
    with open("config/vit.yaml") as f: cfg = yaml.safe_load(f)
    ...
    optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=float(cfg['train']['learning_rate']), weight_decay=float(cfg['train']['weight_decay']))
    ...
    print("--- STARTING ViT LORA TRAINING ---")
    for epoch in range(cfg['train']['epochs']):
        ...
        for imgs, labels in tqdm(train_loader, desc=f"Ep {epoch+1} [Train]"):
            ...
            labels_s = labels * 0.9 + 0.05
            optimizer.zero_grad()
            out = model(imgs)
            loss = criterion(out, labels_s)
        ...
        if m['hter'] < best_hter:
            ...
            patience_cnt += 1
            if patience_cnt >= patience_limit: break
